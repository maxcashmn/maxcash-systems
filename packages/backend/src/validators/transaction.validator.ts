...
export const createTransactionValidator = z.object({
  ...
  currency: z.enum(['USD', 'LRD']).default('USD').optional(),
  method: z.enum(['manual', 'bank_transfer', 'mobile_money', 'cash', 'card']).default('manual').optional(),
  loanId: z.string().uuid('Invalid loan ID format').optional(),
});
